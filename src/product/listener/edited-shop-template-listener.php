...
class Edited_Shop_Template_Listener
{
	/**
	...
	/**
	 * @hook edit_aff_shop_tmpl
	 * @since 0.9.2
	 * @param int $term_id
	 */
	public function before_edit($term_id)
	{
		$term = get_term($term_id, Shop_Template::TAXONOMY, OBJECT);
	...
	public function after_edit($term_id)
	{
		...
		if(isset($this->changed_terms[$term->term_id])) {
			...
			$wpdb->query("
				...
				SET meta_key = REPLACE(meta_key, '_affilicious_product_shops_$old_key', '_affilicious_product_shops_$new_key')
				WHERE meta_key LIKE '_affilicious_product_shops_$old_key-%';
			");
		}
	}
}
