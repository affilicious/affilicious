...
class Admin_Table_Setup
{
    ...
    public function setup_rows($row, $column_name, $term_id)
    {
        ...
        if ($column_name == 'thumbnail') {
            $thumbnail_id = carbon_get_term_meta($term_id, Carbon_Shop_Template_Repository::THUMBNAIL);
        ...
        if ($column_name == 'provider') {
            ...
            if(!empty($provider_id)) {
                $provider = $this->provider_repository->find_by_id(new Provider_Id($provider_id));
